...
import { ittiva_MOCK_API_DEFAULT_DELAY } from '@ittiva/lib/mock-api/mock-api.constants';
import { IttivaMockApiService } from '@ittiva/lib/mock-api/mock-api.service';
import { delay, Observable, of, switchMap, throwError } from 'rxjs';

export const mockApiInterceptor = (request: HttpRequest<unknown>, next: HttpHandlerFn): Observable<HttpEvent<unknown>> =>
{
    const defaultDelay = inject(ittiva_MOCK_API_DEFAULT_DELAY);
    const ittivaMockApiSer = inject(IttivaMockApiService);

    // Try to get the request handler
    const {
        handler,
        urlParams,
    } = ittivaMockApiSer.findHandler(request.method.toUpperCase(), request.url);
