...
// -----------------------------------------------------------------------------------------------------
// @ ittiva TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
    ...
    }) =>
    {
        ...
        addComponents(
            _.fromPairs(_.map(options.themes, (theme, themeName) => [
                ...
                ))), (value, key) => [[`--ittiva-${e(key)}`, value], [`--ittiva-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
            ])),
        );
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`ittiva.customProps.background.${colorScheme}`);
            const foreground = theme(`ittiva.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--ittiva-${e(key)}`, value], [`--ittiva-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--ittiva-${e(key)}`, value], [`--ittiva-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                },
            };
        });
            ...
            theme: {
                extend: {
                    ...
                    colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
                        [name, `rgba(var(--ittiva-${name}-rgb), <alpha-value>)`],
                        [`on-${name}`, `rgba(var(--ittiva-on-${name}-rgb), <alpha-value>)`],
                    ]))),
                },
                ittiva  : {
                    customProps: {
